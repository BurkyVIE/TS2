...
fcts <- list(con = c(BRITAIN = 1L, GERMANY = 2L, USA = 3L, FRANCE = 4L),
             ...
             # col = c(COMMON = "#d6d9de", RARE = "#9cd3fc", EPIC = "#cea6ff", LEGENDARY = "#fbe20b"), # original
             col = c(COMON = "#a6acb7", RARE = "#4db0fa", EPIC = "#a052ff", LEGENDARY = "#ceb903") # 25 % darker
)
...
# TIDY ----
TS2_loco <- read_csv(TS2_loco_raw, col_types = "ciffd", lazy = FALSE) %>% 
...
TS2_loco %>%
  ...
  group_by(Tier, Rating, Power) %>%
  summarise(Count = n(), .groups = "drop") %>%
  ggplot(mapping = aes(x = Rating, y = Power)) +
...
TS2_loco %>%
  mutate(Tier = paste(Tier, "-", Country)) %>% 
  ggplot(mapping = aes(x = Capmax, color = Rating)) +
  geom_density(mapping = aes(fill = Rating), size = 1.5, alpha = .5) +
